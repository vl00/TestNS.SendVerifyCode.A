...
public class Huakuai_v1
{
    ...
    public Func<Task<bool>> CheckMove;

    public Func<string, string> JsfuncOnBeforeEach;
    public Func<string, string> JsfuncOnSetId;
    public string JsBeforeGetImgBytes;
    public string JsOnBgGetImgBytes;
    public string JsAfterGetImgBytes;
    public string JsOnHkGetImgBytes;

    /// <inheritdoc cref="Huakuai_v1"/>
    public Huakuai_v1(string name, IPage page)
    {
        JobName = name;
        this.page = page;
    }
    ...
    /// <inheritdoc cref="Huakuai_v1"/>
    public Huakuai_v1(string name, IFrame iframe)
    {
        JobName = name;
        this.iframe = iframe;
    ...
    public async Task<bool> RunAsync(JobContext ctx, CancellationToken cancellation = default)
    {
...
LB_yanzhengma:
        ...
        await EvaluateFunctionAsync($$"""
                ()=>{
                {{(string.IsNullOrEmpty(JsBeforeGetImgBytes) ? "" : JsBeforeGetImgBytes)}}
                document.querySelector('{{HuaKuaiSelector}}').style.display = 'none';
                document.querySelector('{{BgSelector}}').style.display = 'block';
                {{(string.IsNullOrEmpty(JsOnBgGetImgBytes) ? "" : JsOnBgGetImgBytes)}}
                }
                """);
        await Task.Delay(200, cancellation);
        var bys_bg = await ExGetBytesByScreenshot(BgSelector);
 // for test
        await EvaluateFunctionAsync($$"""
                ()=>{
                document.querySelector('{{HuaKuaiSelector}}').style.display = 'block';
                document.querySelector('{{BgSelector}}').style.display = 'none';
                {{(string.IsNullOrEmpty(JsOnHkGetImgBytes) ? "" : JsOnHkGetImgBytes)}}
                }
                """);
        await Task.Delay(200, cancellation);
        var bys_hk = await ExGetBytesByScreenshot(HuaKuaiSelector);
   // for test
        await EvaluateFunctionAsync($$"""
                ()=>{
                document.querySelector('{{HuaKuaiSelector}}').style.display = 'block';
                document.querySelector('{{BgSelector}}').style.display = 'block';
                {{(string.IsNullOrEmpty(JsAfterGetImgBytes) ? "" : JsAfterGetImgBytes)}}
                }
                """);
